...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    db.run(
  `INSERT INTO reports (topic, style, content, time) VALUES (?, ?, ?, ?)`,
  [topic, style, JSON.stringify(parsed), new Date().toISOString()]
);// 最新在前
...
app.get('/api/history', (req, res) => {
  db.all(`SELECT * FROM reports ORDER BY id DESC LIMIT 20`, [], (err, rows) => {
    if (err) {
